...
app.post("/", (req, res)=>{
    ...
    https.get(url, function(https_res) {
        ...
        if(JSON.parse(data).cod == '404'){return res.send(`${parsedData.message}`)}; 

        res.write("<h1> " + cityName + " weather is " + parsedData.weather[0].description) + "</h1>";
        res.write("<h1> " + cityName + " temp is " + parsedData.main.temp) + "</h1>";
        res.write('  <img src="' + "http://openweathermap.org/img/wn/" + parsedData.weather[0].icon + '.png"' + "/>");
        res.send();
      })
    });
});
...
app.get('/cities/:city_name', (req, res) =>{
    ...
    cityModel.find({name: { $regex : new RegExp(`^${cityName}$`, "i")}}, (err, cities)=>{
        if(err){
            res.send("server error");
        }else{
            return (cities.length !== 0)?
            res.send(JSON.stringify(cities)): 
            res.send(`Weather information for city - <i>${cityName}</i> is not available`); 
        }
    });
});
...
app.delete('/delete/:name', (req,res)=>{
   ...
   cityModel.deleteOne({name: { $regex : new RegExp(`^${cityName}$`, "i")}}, (err, data)=>{
    if(err){
        res.send("server error");
    } else {
        console.log(data);
